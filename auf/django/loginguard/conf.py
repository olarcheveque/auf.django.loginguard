...
LOGIN_GUARD_RETRY_POLICY = getattr(
    settings,
    'LOGIN_GUARD_RETRY_POLICY',
    (
        (60 * 10, 3),  # 3 times allowed in 10min
        (60 * 20, 4),  # 4 times allowed in 20min (+1 extra wait + 10min)
...
LOGIN_GUARD_FREQUENCY_ALERT = getattr(
    settings,
    'LOGIN_GUARD_FREQUENCY_ALERT',
    (
        (3600 * 24 * 3, 10),  # alert if 10 try for same user within 3 days
    ))
